...
const VerificationPage = () => {
  ...
  const isFormValid = legalFiles.length > 0 && trustees.length > 0;

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      setLegalFiles([...legalFiles, ...Array.from(e.target.files)]);
    }
  };
  ...
  const handleAddTrustee = () => {
    if (isTrusteeValid) {
      setTrustees([
        ...trustees,
        { name: trusteeName, phone: trusteePhone, email: trusteeEmail },
      ]);
      setTrusteeName("");
      setTrusteePhone("");
      setTrusteeEmail("");
