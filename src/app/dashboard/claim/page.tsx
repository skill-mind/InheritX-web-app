    ...
    <main className="flex flex-col gap-6 p-4 md:p-8 w-full">
      ...
      <section className="bg-transparent p-0 md:p-2 flex flex-col gap-4">
          ...
          <div className="flex gap-2">
            {tabs.map((tab, idx) => (
              <button
                ...
                className={`px-4 py-1 text-sm cursor-pointer font-medium transition-colors focus:outline-none ${
                  activeTab === idx
        ...
        {activeTab === 0 ? (
          ...
          <div className="flex flex-col bg-[#182024] rounded-[24px] py-[32px] px-[12px] md:px-[24px] min-h-[224px] w-full overflow-x-auto">
            <table className="w-full min-w-[600px]">
              ...
              <tbody>
                {activities.map((item, idx) => (
                  <tr key={idx} className="border-none text-[#FCFFFF] text-[14px]">
                    <td className="py-4 px-2">
                      <span className="text-[14px]">{idx + 1}. {item.activity}</span>
                    </td>
                    <td className="py-4 px-2 text-[#92A5A8] text-[13px]">{item.timestamp}</td>
                  </tr>
                ))}
              </tbody>
            </table>
