...
const thread = [
  {
    sender: "Admin",
    avatar: "/assets/icons/white_logo.svg",
    text: "We have checked and found nothing wrong with the transaction",
    role: "admin",
  },
  {
    sender: "Reporter",
    avatar: "/assets/icons/avatar.svg",
    text: "I think this is a fraudulent withdrawal case. Please check again",
    role: "reporter",
  },
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#161E22]/80 backdrop-blur-[6px]">
      <div className="relative w-full max-w-2xl mx-4 md:mx-0 md:w-[700px] rounded-[24px] bg-[#161E22] border border-[#2A3338] p-6 md:p-10 shadow-xl z-10 flex flex-col gap-6 max-h-[95vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-2">
          <span
            onClick={onClose}
            className="text-[#92A5A8] text-[14px] font-medium flex items-center cursor-pointer"
          >
            <Image
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full text-xs font-semibold bg-yellow-900/30 text-yellow-300 border border-yellow-700/30">
              PENDING
            </span>
        ...
        {/* Request Summary */}
        <div>
          <h2 className="text-[#FCFFFF] text-[16px] font-medium mb-6">
            Request Summary
          </h2>
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[14px] mb-6">
            ...
            <span className="text-[#33C5E0] underline cursor-pointer">
              Shally poppy
            </span>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[#FCFFFF] text-[13px]">
              July 15th, 2025 11:59PM
            </span>
          </div>
        </div>
        {/* Sections */}
        <div className="flex flex-col gap-2">
          {/* Attached Documents */}
          <button
            className="w-full flex justify-between items-center py-3 px-4 bg-[#182024] rounded-[12px] text-[#BFC6C8] text-[14px] font-normal"
            ...
            <span className="ml-2">
              <Image
                ...
                className={`transition-transform duration-200 ${
                  expanded === "docs" ? "rotate-180" : "rotate-0"
                }`}
              />
            </span>
          </button>
              ...
              {activeDoc !== null ? (
                <div className="w-full flex flex-col items-center mb-6">
                  <div className="w-[320px] h-[200px] rounded-[12px] overflow-hidden border border-[#33C5E0] bg-[#181F24] flex items-center justify-center relative mb-2 shadow-[0_0_24px_0_#33C5E0]">
                    <Image
                      src={documents[activeDoc].src}
                      alt={documents[activeDoc].label}
                      width={320}
                      height={200}
                      className="object-contain"
                    />
                    <button
                      className="absolute top-2 right-2 text-[#33C5E0] bg-[#161E22] rounded-full px-2 py-1 text-xs font-bold shadow"
                      onClick={() => setActiveDoc(null)}
                    >
                      Close
                    </button>
                  </div>
                  <span className="text-[#FCFFFF] text-[13px] mt-1 text-center font-semibold">
                  ...
                  <div className="flex gap-2 mt-4">
                    {documents.map((doc, idx) => (
                      <button
                        key={idx}
                        className={`w-[60px] h-[40px] rounded-[6px] overflow-hidden border ${
                          activeDoc === idx
                            ? "border-[#33C5E0]"
                            : "border-[#222C32]"
                        } bg-[#181F24] flex items-center justify-center relative transition-all duration-150 cursor-pointer hover:scale-105`}
                  ...
                  >
                    <div className="w-[110px] h-[70px] rounded-[8px] overflow-hidden border border-[#222C32] bg-[#181F24] flex items-center justify-center relative hover:border-[#33C5E0] transition-all duration-150">
                      <Image
                        ...
                        className="object-contain"
                      />
                      <span className="absolute bottom-2 right-2 text-[#33C5E0] text-xs">
                        ↗
                      </span>
                    </div>
          ...
          >
            ...
            <span className="ml-2">
              <Image
                ...
                height={6}
                className={`transition-transform duration-200 ${
                  expanded === "thread" ? "rotate-180" : "rotate-0"
                }`}
              />
            </span>
          </button>
                ...
                >
                  <div
                    className={`flex items-center gap-3 ${
                      msg.role === "admin"
                        ? "bg-[#1C252A] border-[] border-l-[4px] border-l-[#33C5E03D]"
                        : "bg-[#1C252A] border border-[#33C5E03D] border-r-[4px] border-r-[#33C5E03D]"
                    } rounded-[12px] px-4 py-3 max-w-[80%]`}
                  >
                    <div className="flex flex-col items-center gap-2 mr-3">
                      <div className="w-[32px] h-[32px] rounded-full p-2 border border-[#33C5E03D] bg-[#33C5E014]">
                        <Image
                          src={msg.avatar}
                          alt={msg.sender}
                          width={20}
                      ...
                      <span className="text-[#92A5A8] text-xs">
                        {msg.role === "admin" ? "Admin" : "Reporter"}
                      </span>
                    </div>
                    <div>
                      <div className="text-[#BFC6C8] text-xs font-medium mb-1 flex items-center gap-1"></div>
                      <div className="text-[#92A5A8] text-sm">{msg.text}</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          >
            ...
            <span className="ml-2">
              <Image
                ...
                height={6}
                className={`transition-transform duration-200 ${
                  expanded === "notes" ? "rotate-180" : "rotate-0"
                }`}
              />
            </span>
          </button>
          {expanded === "notes" && (
            <div className="bg-[#182024] rounded-[12px] py-4 px-2 md:px-6 mt-3 mb-2 flex flex-col gap-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <button className="text-[#BFC6C8] cursor-pointer text-xs flex items-center gap-1">
                  <Image
              ...
              <textarea
                className="w-full bg-[#161E22] rounded-[8px] p-2 text-[#FCFFFF] text-sm resize-none min-h-[40px]"
                defaultValue={notes}
              />
            </div>
          )}
        ...
        <div className="flex flex-col md:flex-row gap-3 md:gap-4 mt-2">
          <button
            className="flex-1 cursor-pointer bg-[#222C32] text-[#FCFFFF] hover:bg-[#1C252A] rounded-full py-3 font-medium text-[14px] border border-[#222C32] flex items-center justify-center"
          ...
          <button
            className="flex-1 cursor-pointer bg-[#33C5E0] text-[#161E22] hover:bg-cyan-600 rounded-full py-3 font-semibold text-[15px] border border-[#33C5E0] flex items-center justify-center"
            onClick={onClose}
          >
