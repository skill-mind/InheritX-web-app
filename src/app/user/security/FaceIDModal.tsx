...
const FaceIDModal: React.FC<FaceIDModalProps> = ({ open, onClose, onContinue }) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#161E22CC] backdrop-blur-sm">
      <div className="bg-[#161E22] border border-[#2A3338] rounded-2xl p-8 md:p-12 w-full max-w-3xl flex flex-col items-center shadow-xl relative mx-2">
        ...
        <h2 className="text-xl md:text-2xl font-medium text-[#FCFFFF] mb-2 mt-2 text-center">Enable Face ID</h2>
        <p className="text-[#92A5A8] text-[13px] md:text-[15px] text-center mb-8 font-normal">Please look straight into the camera and smile</p>
        <div className="flex flex-col items-center mb-10">
          <div className="bg-white rounded-xl p-3 md:p-4 mb-2 flex items-center justify-center border-2 border-[#33C5E0]">
            <Image src="/assets/images/faceid.svg" alt="faceid" width={200} height={200} className="w-[180px] h-[180px] md:w-[220px] md:h-[220px] object-contain" />
          </div>
        </div>
        <button
